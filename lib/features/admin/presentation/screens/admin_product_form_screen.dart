...
class _AdminProductFormScreenState extends ConsumerState<AdminProductFormScreen> {
  ...
  void dispose() {
    ...
    _price.dispose(); _discount.dispose(); _sku.dispose(); _slug.dispose();
    super.dispose();
  }
            ...
            _buildSection(isAr ? 'القسم' : 'Category', isDark, [
              categoriesAsync.when(
                data: (cats) => DropdownButtonFormField<String>(
                  ...
                  items: cats.map((c) => DropdownMenuItem(value: c['id'] as String, child: Text(c[isAr ? 'name_ar' : 'name_en'] as String? ?? '', style: const TextStyle(fontFamily: 'Outfit')))).toList(),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final data = {
        ...
        'slug': _slug.text.trim().isNotEmpty ? _slug.text.trim() : _nameEn.text.trim().toLowerCase().replaceAll(' ', '-'),
      ...
      String productId;
      if (_isEditing) {
        await ds.updateProduct(widget.product!['id'] as String, data);
        productId = widget.product!['id'] as String;
      } else {
        final result = await ds.createProduct(data);
